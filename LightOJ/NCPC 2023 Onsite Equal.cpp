...
// https://www.geeksforgeeks.org/bigint-big-integers-in-c-with-example/ (Customized in ostream, istream and added BigInt(const BigInt &);)
class BigInt{
    string digits;
public:
    //Constructors:
...
void solve()
{
    ...
    // BigInt arr[] = {BigInt(a), BigInt(b), BigInt(c), BigInt(d)};
    vector<BigInt> arr = {a, b, c, d};

    BigInt product1, product2;
    product1 = a * d;
    product2 = b * c;

    if (product1 == product2) cout << "Equal" << endl;
    else cout << "Not Equal" << endl;

    // This problem can also be solved by (Multiply Large Numbers represented as Strings). Check GFG
}
